...
/* Create a bunch of protothreads that block in the kernel, which should
 * cause the max number of pthreads (20) to be created.
 *
 * If this test fails, it may be that the usleep values are slightly off.
 */
static pt_t
...
#define TEST_ITEMS() \
    item(test_create_dynamic,   true) \
    item(test_create_static,    true) \
    item(test_thread_create,    true) \
    item(test_blocking,         true) \
    item(test_yield,            true) \
    item(test_broadcast,        true) \
    item(test_pc,               true) \
    item(test_pc_big,           true) \
    item(test_recursive,        true) \
    item(test_func_pointer,     true) \
...
main(int argc, char **argv)
{
    ...
    {
        int r = pthread_mutex_init(&app_mutex, NULL) ;
        assert(!r) ;
    }
    srand(getpid()) ;
#define item(test, enabled) \
    printf(#test " ... ") ; \
    if (!enabled) { \
        printf("skipping\n") ; \
    } else { \
